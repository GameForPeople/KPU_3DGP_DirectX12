...
CHeightMapGridMesh::CHeightMapGridMesh(ID3D12Device *pd3dDevice,
	ID3D12GraphicsCommandList *pd3dCommandList, int xStart, int zStart, int nWidth, int
	nLength, XMFLOAT3 xmf3Scale, XMFLOAT3 xmf3Normal, void *pContext) : CMesh(pd3dDevice,
		pd3dCommandList)
{
	//격자의 교점(정점)의 개수는 (nWidth * nLength)이다.
	m_nVertices = nWidth * nLength;
	...
	for (int i = 0, z = zStart; z < (zStart + nLength); z++)
	{
		for (int x = xStart; x < (xStart + nWidth); x++, i++)
		{
			...
			XMFLOAT3 xmf3Position = XMFLOAT3((x*m_xmf3Scale.x), OnGetHeight(x, z, pContext),
				(z*m_xmf3Scale.z));
			XMFLOAT3 xmf3Normal = Vector3::Add(OnGetNormal(x, z, pContext), xmf3Normal);
...
CWallMeshIlluminated::CWallMeshIlluminated(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList
	*pd3dCommandList, float fWidth, float fHeight, float fDepth, int value) : CMeshIlluminated(pd3dDevice, pd3dCommandList)
{
	//직육면체는 꼭지점(정점)이 8개이다.
	m_nVertices = 8;
	m_nStride = sizeof(CIlluminatedVertex);
	...
	m_pNewVertices = new CIlluminatedVertex[m_nVertices];

	//m_pNewVertices[0] = CIlluminatedVertex(XMFLOAT3(-fx, +fy, -fz), RANDOM_COLOR);
	//m_pNewVertices[1] = CIlluminatedVertex(XMFLOAT3(+fx, +fy, -fz), RANDOM_COLOR);
	//m_pNewVertices[2] = CIlluminatedVertex(XMFLOAT3(+fx, +fy, +fz), RANDOM_COLOR);
	//m_pNewVertices[3] = CIlluminatedVertex(XMFLOAT3(-fx, +fy, +fz), RANDOM_COLOR);
	//m_pNewVertices[4] = CIlluminatedVertex(XMFLOAT3(-fx, -fy, -fz), RANDOM_COLOR);
	//m_pNewVertices[5] = CIlluminatedVertex(XMFLOAT3(+fx, -fy, -fz), RANDOM_COLOR);
	//m_pNewVertices[6] = CIlluminatedVertex(XMFLOAT3(+fx, -fy, +fz), RANDOM_COLOR);
	//m_pNewVertices[7] = CIlluminatedVertex(XMFLOAT3(-fx, -fy, +fz), RANDOM_COLOR);
	for ( int i = 0 ; i < 8; i++)
		m_pNewVertices[i] = CIlluminatedVertex(pxmf3Positions[i], pxmf3Normals[i]);
