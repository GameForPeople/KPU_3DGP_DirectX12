...
CCamera *CPlayer::OnChangeCamera(DWORD nNewCameraMode, DWORD nCurrentCameraMode)
{
	...
	if (pNewCamera)
	{
		...
		pNewCamera->SetOffset(XMFLOAT3(0.0f, 50.0f, -5.0f));
	}
	if (m_pCamera) delete m_pCamera;
	return(pNewCamera);
}
...
축 방향으로 향하도록 그릴 것이기 때문이다   실제업무에서는!!!!!!!!!!!!! 당연히 모델링 제대로 된걸로!!오프라인!!.*/
...
CCamera *CAirplanePlayer::ChangeCamera(DWORD nNewCameraMode, float fTimeElapsed)
{
	...
	case THIRD_PERSON_CAMERA:
		...
		m_pCamera->SetOffset(XMFLOAT3(0.0f, 50.0f, -5.0f));
		m_pCamera->GenerateProjectionMatrix(1.01f, 5000.0f, ASPECT_RATIO, 60.0f);
		m_pCamera->SetViewport(0, 0, FRAME_BUFFER_WIDTH, FRAME_BUFFER_HEIGHT, 0.0f,
			1.0f);
		m_pCamera->SetScissorRect(0, 0, FRAME_BUFFER_WIDTH, FRAME_BUFFER_HEIGHT);
