...
CFaceMeshTextured::~CFaceMeshTextured()
{
}
...
//	m_nStride = sizeof(CTexturedVertex);
#ifdef NEW_CODE_1
	m_nStride = sizeof(CDiffused3TexturedVertex);
#else	
...
#ifdef NEW_CODE_1
	CDiffused3TexturedVertex *pVertices = new CDiffused3TexturedVertex[m_nVertices];
#else
...
#ifdef NEW_CODE_1
			pVertices[i].m_xmf2TexCoord2 = XMFLOAT2(float(x) / float(m_xmf3Scale.x*0.7f), float(z) / float(m_xmf3Scale.z*0.7f));
#endif
...
#ifdef NEW_CODE_3
CTexturedRectBoardMesh::CTexturedRectBoardMesh(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, float fWidth, float fHeight, float fDepth, float fxPosition, float fyPosition, float fzPosition) : CMesh(pd3dDevice, pd3dCommandList)
{
	m_nVertices = 6;
	m_nStride = sizeof(CTexturedVertex);
	m_d3dPrimitiveTopology = D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST;
...
CTexturedRectBoardMesh::~CTexturedRectBoardMesh()
{
}
#endif